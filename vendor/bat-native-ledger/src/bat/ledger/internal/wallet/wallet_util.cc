...
type::ExternalWalletPtr GetWallet(LedgerImpl* ledger, const std::string wallet_type) {
  DCHECK(ledger);
  const std::string wallet_string =
      ledger->ledger_client()->GetEncryptedStringState(WalletTypeToState(wallet_type));
...
bool SetWallet(LedgerImpl* ledger, type::ExternalWalletPtr wallet, const std::string state) {
  DCHECK(ledger);
  if (!wallet) {
    return false;
  }
...
type::ExternalWalletPtr ResetWallet(type::ExternalWalletPtr wallet, const std::string wallet_type) {
  if (!wallet) {
    return nullptr;
  }
