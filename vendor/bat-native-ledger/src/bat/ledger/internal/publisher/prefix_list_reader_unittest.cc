...
TEST_F(PrefixListReaderTest, BrotliCompression) {
  ...
  constexpr unsigned char compressed[] = {
  ...
  size_t compressed_length = sizeof(compressed) / sizeof(unsigned char);
  std::string prefixes(&compressed[0], &compressed[0] + compressed_length);
