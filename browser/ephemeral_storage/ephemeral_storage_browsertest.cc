...
HttpRequestMonitor::~HttpRequestMonitor() =
    default;

void HttpRequestMonitor::OnHttpRequest(
    const HttpRequest& request) {
...
// Test for Android-specific bug when a renderer caches localStorage proxy in
// the same process without a proper cleanup.
